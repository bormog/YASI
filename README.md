...
Чем занимается [Frontend?](https://ps-group.github.io/compilers/what_is_frontend)
 ...
 - Строит [Абстрактное Синтаксическое Дерево](https://ps-group.github.io/compilers/ast.html), хранящее логическую модель исходного файла
...
За эту часть отвечают сущности: TokenType, Token, Lexer
 ...
 - TokenType - нужен для перечисления всех возможных токенов
   + Наследуется от Enum
   + Каждому токену сопоставляется регулярка
   + Конец строки обозначается как EOF
 - Token - токен. У токена есть:
   + token_type - тип токена (TokenType)
   + value - значение которое было считано из текста
   + position -  позиция токена в тексте. Позиция нужна в синтаксическом анализе, чтобы показать место в котором произошла ошибка
 - Lexer - преобразует SQL текст в "список" Token-ов.
   + Парсит текст в один проход
   + Сделан как генератор, чтобы при последующем использовании можно было так же интерпретировать логику программы в один проход  
   + Умеет отдавать ошибку, если исходный текст содержит незнакомый символ
...
За эту часть отвечают сущности: Parser, Nodes
 - Parser - реализует [метод рекурсивного спуска](https://ru.wikipedia.org/w/index.php?title=%D0%9C%D0%B5%D1%82%D0%BE%D0%B4_%D1%80%D0%B5%D0%BA%D1%83%D1%80%D1%81%D0%B8%D0%B2%D0%BD%D0%BE%D0%B3%D0%BE_%D1%81%D0%BF%D1%83%D1%81%D0%BA%D0%B0)
   + Получает на вход машинную ленту с токенами (Lexer)
   + Каждое правило [грамматики](https://ps-group.github.io/compilers/grammars.html) - реализовано в виде отдельного метод. 
   + Парсер считывает с машинной ленты по одному токену и рекурсивно идет по правилам грамматики
   + На выходе отдает AST дерево  
 ...
 - Nodes - реализация AST дерева. Для каждой сущности создается своя нода.
   + Фактически AST дерево - это исходная программа представленная в виде правил грамматики
   + Дерево не содержит лишние символы, к-ые не нужны для понимания логики. Например это скобки, или ключевые слова такие как insert, order
   + Каждая нода дерева это либо инструкция (set, update, insert), либо выражение (1+1, foo=3), либо объявление (таблица, колонка, переменная) 
...
За эту часть отвечают сущности: Interpreter, Storage, Table
 - Interpreter - умеет обходить дерево и умеет понимать каким конкретно образом нужно обработать каждую ноду
   + Для каждой ноды у этого класса есть метод visit, в котором запрограммирована логика того как конкретно интерпретировать ноду
   + Арифметические операции вычисляются напрямую при обходе дерева в методe visit_binary_operation
   + Чтобы работать с таблицами - интерпретатор дергает класс Storage 
 - Storage - псевдо-реализация базы данных.
 - Table - Абстракция над таблицами
   + Данные сохраняются в CSV файл
   + При поиске учитывается индекс. Т.е если идет поиск по нескольким полям и среди них есть индекс, то в первую очередь будет сделана выборка по индекса.
   + Для работы с CSV используется Pandas
